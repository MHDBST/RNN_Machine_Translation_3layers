...
class Sampler(SimpleExtension, SamplingBase):
    ...
    def do(self, which_callback, *args):
        ...
        src_reorder_batch = batch[self.main_loop.data_stream.mask_sources[2]]

        input_ = src_batch[sample_idx, :]
        target_ = trg_batch[sample_idx, :]
        input_reorder_ = src_reorder_batch[sample_idx, :]
        ...
        for i in range(hook_samples):
            ...
            input_reorder_length = self._get_true_length(input_reorder_[i], self.src_vocab)

            inp = input_[i, :input_length]
            inp_reorder = input_[i, :input_reorder_length]


            _1, outputs, _2, _3, costs = (self.sampling_fn(inp[None, :],inp_reorder[None, :]))
...
class BleuValidator(SimpleExtension, SamplingBase, SaveLoadUtils):
    ...
    def __init__(self, source_sentence,source_sentence_reorder, samples, model, data_stream,
                 config, n_best=1, track_n_models=1,
                 normalize=True, **kwargs):
        # TODO: change config structure
        super(BleuValidator, self).__init__(**kwargs)
        self.source_sentence = source_sentence
        self.source_sentence_reorder = source_sentence_reorder
        ...
        # Helpers
        self.vocab = data_stream[0].dataset.dictionary
        self.unk_sym = data_stream[0].dataset.unk_token
        self.eos_sym = data_stream[0].dataset.eos_token
    ...
    def _evaluate_model(self):
        ...
        print("before_evaluate_model")
        mb_subprocess = Popen(self.multibleu_cmd, stdin=PIPE, stdout=PIPE)
        print("after_evaluate_model")
        total_cost = 0.0

        # Get target vocabulary
        print(1)
        sources = self._get_attr_rec(self.main_loop, 'data_stream')#       sources = sources.data_streams[0:1]
        ...
        for i, (line,line_reorder) in enumerate(zip(self.data_stream[0].get_epoch_iterator(), self.data_stream[1].get_epoch_iterator())):
            # print("khate " , i )
            # print("line" , line[0])
            # print("line_reo", line_reorder[0])
            """
            ...
            seq_reorder = self._oov_to_unk(
                line_reorder[0], self.config['src_vocab_size'], self.unk_idx)
            input_reorder_ = numpy.tile(seq_reorder, (self.config['beam_size'], 1))


            # print("self.source_sentence" , self.source_sentence)

            # draw sample, checking to ensure we don't get an empty string back
            trans, costs = \
                self.beam_search.search(
                    input_values={self.source_sentence: input_,self.source_sentence_reorder: input_reorder_},
        ...
        self.data_stream[0].reset()
        self.data_stream[1].reset()
