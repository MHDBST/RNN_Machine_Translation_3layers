...
def get_tr_stream(src_vocab, trg_vocab, src_data, trg_data,src_data_reorder,
    ...
    src_dataset_reorder =TextFile([src_data_reorder], src_vocab, None)

    # Merge them to get a source, target pair
    stream = Merge([src_dataset.get_example_stream(),
                    trg_dataset.get_example_stream(),
                   src_dataset_reorder.get_example_stream()],
                   ('source', 'target','source_reorder'))
    ...
    masked_stream = PaddingWithEOS(
        stream, [src_vocab_size - 1, trg_vocab_size - 1,src_vocab_size - 1])

    return masked_stream


def get_dev_stream(val_set=None,val_set_reorder=None, src_vocab=None, src_vocab_size=30000,
                   unk_id=1, **kwargs):
    # """Setup development set stream if necessary."""
    all_data = None
    if val_set is not None and val_set_reorder is not None and src_vocab is not None:
        src_vocab = _ensure_special_tokens(
            src_vocab if isinstance(src_vocab, dict) else
            cPickle.load(open(src_vocab)),
            bos_idx=0, eos_idx=src_vocab_size - 1, unk_idx=unk_id)
        dev_dataset = TextFile([val_set], src_vocab, None)
        dev_reorder = TextFile([val_set_reorder], src_vocab, None)
        from theano import tensor
        from picklable_itertools.extras import equizip

        # all_data = conc([dev_dataset,dev_reorder],('dev_source', 'dev_reorder'))
        dev_stream = DataStream(dev_dataset)
        dev_stream_reorder = DataStream(dev_reorder)
        # all_data = [tensor.concatenate([dev_stream, dev_stream_reorder])]
        # all_data = Merge([dev_stream,dev_stream_reorder],
        #       ('dev', 'dev_reorder'))
        all_data = [dev_stream,dev_stream_reorder]
    return all_data
    # """Setup development set stream if necessary."""
    # dev_stream = None
