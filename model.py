...
from myArch import GatedRecurrent
from myMisc import  Bidirectional
...
class BidirectionalWMT15(Bidirectional):
    ...
    @application
    def apply(self, forward_dict,backward_dict,reorder_dict):
        """Applies forward and backward networks and concatenates outputs."""
        forward = self.children[0].apply(as_list=True, **forward_dict)
        backward = [x[::-1] for x in
                    self.children[1].apply(reverse=True, as_list=True,
                                           **backward_dict)]
        reorder = self.children[2].apply(as_list=True, **reorder_dict)

        return [tensor.concatenate([f,  b,r], axis=2)
                for f,  b, r in equizip(forward,  backward, reorder)]
...
class BidirectionalEncoder(Initializable):
    ...
    def __init__(self, vocab_size, embedding_dim, state_dim, **kwargs):
        ...
        #
        # self.reorder_fork = Fork(
        #         [name for name in self.bidir.prototype.apply.sequences
        #          if name != 'mask'], prototype=Linear(), name='reorder_fork')
    ...
    def _push_allocation_config(self):
        ...
        # self.reorder_fork.input_dim = self.embedding_dim
        # self.reorder_fork.output_dims = [self.bidir.children[2].get_dim(name)
        #                               for name in self.reorder_fork.output_names]

    @application(inputs=['source_sentence', 'source_sentence_mask','source_sentence_reorder', 'source_sentence_reorder_mask'],
                 outputs=['representation'])
    def apply(self, source_sentence, source_sentence_mask,source_sentence_reorder, source_sentence_reorder_mask):
        ...
        source_sentence_reorder = source_sentence_reorder.T
        source_sentence_reorder_mask = source_sentence_reorder_mask.T

        embeddings_reorder = self.lookup.apply(source_sentence_reorder)
        ...
        representation = self.bidir.apply(
            ...
            merge(self.back_fork.apply(embeddings, as_dict=True),
                  {'mask': source_sentence_mask}) ,
            merge(self.back_fork.apply(embeddings_reorder, as_dict=True),
                  {'mask': source_sentence_reorder_mask})
        )
